...
Package scope: `@appetitebind` (override via `package_scope` in config when cloning).

## HARD RULE — never commit or push `.codegen`

`.codegen/` is **local tooling only**. It must never be staged, committed, or pushed to GitHub (same for `codegen/` and `**/zero_codegen/`).

1. Confirm `.gitignore` contains `.codegen/` before any commit.
2. Refuse `git add .codegen` / `git add -f .codegen`.
3. If `.codegen` is missing locally, copy it from `zero-apps-codegen-scaffold` (or a sibling product), then run `pnpm codegen:paths`.
...
2. Set `package_scope` (e.g. `@acme`) and rename `@appetitebind/*` packages.
