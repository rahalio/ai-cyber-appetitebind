...
{repo}/
  ...
  .codegen/           # local zero-codegen tool — NEVER commit or push
```

## HARD RULE — never push `.codegen`

`.codegen/` (and `codegen/`, `**/zero_codegen/`) must not be committed. Verify `.gitignore` before staging. Bootstrap locally from `zero-apps-codegen-scaffold` when the folder is missing.
