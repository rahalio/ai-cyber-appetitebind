 ...
 * IssueRepositoryDdb
 *
 * DDD: Infrastructure adapter implementing Issue repository using DynamoDB.
 * Generated from x-dynamodb metadata in OpenAPI schema.
 */

import type { IssueRepository } from "@appetitebind/services/governance";
...
/**
 * IssueRepositoryDdb using DynamoDB table (type: core)
 *
 * Entity Type: ISSUE
 * Entity Token: ISSUE
 * SK Prefix: ISSUE
...
export class IssueRepositoryDdb implements IssueRepository {
  ...
  private readonly ENTITY_TYPE = "ISSUE";
  private readonly ENTITY_TOKEN = "ISSUE";
  private readonly SK_PREFIX = "ISSUE";
  ...
  private readonly ENTITY_TYPE_PK = "ISSUES";
  ...
  async openIssue(input: Parameters<IssueRepository['openIssue']>[0]): Promise<Awaited<ReturnType<IssueRepository['openIssue']>>> {
    ...
    if (!(inputExt.issueId ?? inputExt.id ?? inputExt.issueId)) {
      throw new Error("Missing required parameter: issueId");
    }
    if (!inputExt.orgId) {
      throw new Error("Missing required parameter: orgId");
    }
    const pk = this.buildPK(inputExt.orgId as string, (inputExt.issueId ?? inputExt.id ?? inputExt.issueId) as string);
    // Soft-delete layout prefixes SK with ACTIVE# when statusPrefix is enabled.
    const skPrefix = `${this.ENTITY_TOKEN}#${(inputExt.issueId ?? inputExt.id ?? inputExt.issueId)}#`;
    ...
    if (!item) {
      throw new Error(`${this.ENTITY_TYPE} with id ${(inputExt.issueId ?? inputExt.id ?? inputExt.issueId)} not found`);
    }
    return {
