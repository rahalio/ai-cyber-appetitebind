 ...
 * MetricRepositoryDdb
 *
 * DDD: Infrastructure adapter implementing Metric repository using DynamoDB.
 * Generated from x-dynamodb metadata in OpenAPI schema.
 */

import type { MetricRepository } from "@appetitebind/services/monitoring";
...
/**
 * MetricRepositoryDdb using DynamoDB table (type: core)
 *
 * Entity Type: MONITORING_METRIC
 * Entity Token: MONITORING_METRIC
 * SK Prefix: MONITORING_METRIC
 */
export class MetricRepositoryDdb implements MetricRepository {
  ...
  private readonly ENTITY_TYPE = "MONITORING_METRIC";
  private readonly ENTITY_TOKEN = "MONITORING_METRIC";
  private readonly SK_PREFIX = "MONITORING_METRIC";
  ...
  private readonly ENTITY_TYPE_PK = "MONITORING_METRICS";
  ...
  async ingestMonitoringMetric(input: Parameters<MetricRepository['ingestMonitoringMetric']>[0]): Promise<Awaited<ReturnType<MetricRepository['ingestMonitoringMetric']>>> {
    const inputExt = input as Record<string, unknown>;

    if (!input) {
      throw new Error("Missing required parameter: input");
    }
    if (!(inputExt.metricId ?? inputExt.id ?? inputExt.metricId)) {
      throw new Error("Missing required parameter: metricId");
    }
    if (!inputExt.orgId) {
      throw new Error("Missing required parameter: orgId");
    }
    const pk = this.buildPK(inputExt.orgId as string, (inputExt.metricId ?? inputExt.id ?? inputExt.metricId) as string);
    // Soft-delete layout prefixes SK with ACTIVE# when statusPrefix is enabled.
    const skPrefix = `${this.ENTITY_TOKEN}#${(inputExt.metricId ?? inputExt.id ?? inputExt.metricId)}#`;
    ...
    if (!item) {
      throw new Error(`${this.ENTITY_TYPE} with id ${(inputExt.metricId ?? inputExt.id ?? inputExt.metricId)} not found`);
    }
    return {
