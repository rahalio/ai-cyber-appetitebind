 ...
 *
 * DDD: Infrastructure adapter implementing UseCase repository using DynamoDB.
 * Generated from x-dynamodb metadata in OpenAPI schema.
 */

import type { UseCaseRepository } from "@appetitebind/services/usecases";
...
/**
 * UseCaseRepositoryDdb using DynamoDB table (type: core)
 *
 * Entity Type: USECASE
 * Entity Token: USECASE
 * SK Prefix: USECASE
 */
export class UseCaseRepositoryDdb implements UseCaseRepository {
  ...
  private readonly ENTITY_TYPE = "USECASE";
  private readonly ENTITY_TOKEN = "USECASE";
  private readonly SK_PREFIX = "USECASE";
  ...
  private readonly DOMAIN = "USECASES";
  private readonly ENTITY_TYPE_PK = "USECASES";
  private generateCorrelationId(): string {
    const domainPrefix = "ucs";
  ...
  async registerAiUseCase(input: Parameters<UseCaseRepository['registerAiUseCase']>[0]): Promise<Awaited<ReturnType<UseCaseRepository['registerAiUseCase']>>> {
    ...
    const ownerId = inputExt?.ownerId ?? inputExt?.id;
    if (!ownerId) throw new Error("Missing required parameter: ownerId (inject from use case)");
    const pk = this.buildPK(orgId as string, ownerId as string);
    const sk = this.buildSK(ownerId as string);
    const excludedFields = ["ownerId" as string, "id"];
  ...
  async getAiUseCase(input: Parameters<UseCaseRepository['getAiUseCase']>[0]): Promise<Awaited<ReturnType<UseCaseRepository['getAiUseCase']>>> {
    ...
    if (!(inputExt.ownerId ?? inputExt.id ?? inputExt.useCaseId)) {
      throw new Error("Missing required parameter: ownerId");
    }
    if (!inputExt.orgId) {
      throw new Error("Missing required parameter: orgId");
    }
    const pk = this.buildPK(inputExt.orgId as string, (inputExt.ownerId ?? inputExt.id ?? inputExt.useCaseId) as string);
    // Soft-delete layout prefixes SK with ACTIVE# when statusPrefix is enabled.
    const skPrefix = `${this.ENTITY_TOKEN}#${(inputExt.ownerId ?? inputExt.id ?? inputExt.useCaseId)}#`;
    ...
    if (!item) {
      throw new Error(`${this.ENTITY_TYPE} with id ${(inputExt.ownerId ?? inputExt.id ?? inputExt.useCaseId)} not found`);
    }
    return {
      data: sanitizeItem(item as Record<string, unknown>) as unknown as any,
      meta: {
